...
if (shake_timer > 0) {
    shake_timer--;
    shake_intensity *= 0.82;
}

// Controlla danno dal player
var player = instance_find(obj_player, 0);
if (player != noone) {
    // SOLO se il player sta usando il PICCONE (tool 1)
    if (variable_instance_exists(player, "is_mining") && player.is_mining && global.selected_tool == 1) {
        var my_dist = point_distance(x, y, player.x, player.y);

        // SOLO se questa specifica rock è vicina
        if (my_dist < 50) {
            if (current_time - last_damage_time > 500) {
                ...
                if (random(100) < 50) {
                    ...
                    drop.vspeed = random_range(-2, -0.8);
                }
            }
        }
    }
